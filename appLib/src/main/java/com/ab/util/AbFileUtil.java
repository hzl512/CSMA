...
public class AbFileUtil {
	...
	/** 默认语音文件目录. */
	private static  String wechatVoiceDownloadDir = null;
   ...
   public static void copyAssets2SD(Context context, String assetDir, String outDir) {
	...
	public static void initFileDir(Context context){
		...
		//默认语音下载与上传目录.
		String wechatVoiceDownloadPath = downloadRootPath + AbAppConfig.WECHAT_VOICE_DIR + File.separator;
		...
		try {
			if(!isCanUseSD()){
				...
				File weChatVoiceDownloadDirFile = new File(root.getAbsolutePath() + wechatVoiceDownloadPath);
				if(!weChatVoiceDownloadDirFile.exists()){
					weChatVoiceDownloadDirFile.mkdirs();
				}
				wechatVoiceDownloadDir = weChatVoiceDownloadDirFile.getPath();
	...
	/**
	 * Gets the weChatVoice download dir.
	 *
	 * @param context the context
	 * @return the weChatVoice download dir
	 */
	public static String getweChatVoiceDownloadDir(Context context) {
		if(wechatVoiceDownloadDir == null){
			initFileDir(context);
		}
		return wechatVoiceDownloadDir;
	}
