...
import com.ab.fragment.AbAlertDialogNegativeFragment;
...
public class AbDialogUtil {
     ...
     * @param onClickListener 点击确认按钮的事件监听
     */
    public static AbAlertDialogNegativeFragment showAlertDialog(Context context,String title,String message,AbAlertDialogNegativeFragment.AbDialogNegativeOnClickListener onClickListener) {
        FragmentActivity activity = (FragmentActivity)context;
        AbAlertDialogNegativeFragment newFragment = AbAlertDialogNegativeFragment.newInstance(0,title,message,null,onClickListener);
        FragmentTransaction ft = activity.getFragmentManager().beginTransaction();
        // 指定一个系统转场动画
        ft.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_OPEN);
        newFragment.show(ft, mDialogTag);
